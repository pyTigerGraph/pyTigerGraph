...
class Featurizer:
    '''Install and run different graph algorithms available in graph data science library.'''
    def __init__(
    self, 
    ...
    local_gsql_path: str = None):
        ...
        The job of a feature extracter is to install and run the current algoriths in graph data science libarary.
        A set of graph algorithms are moved to the gsql folder and have been saved into a dictionary along with their output type.
        ...
        Args:
            ...
            result_attr (str): An attribute name that needs to be added to the vertex/edge
            local_gsql_path (str): A path to where the algorithm. is located
        """

        self.conn = conn
        self.name_of_query = name_of_query
        self.result_attr = result_attr
        self.local_gsql_path = local_gsql_path
    ...
    def _install_query_file(self, file_path: str, replace: dict = None):
        ...
        self.local_gsql_path = file_path
        with open(file_path) as infile:
            firstLine = infile.readline()
        try:
            name_of_query = firstLine.split("QUERY")[1].strip().split("(")[0]
        except:
            raise ValueError("Cannot parse the query file. The query file should start with CREAT QUERY query_name ...")
    ...
    def installAlgorithm(self,query_name:str,schema_type:str="VERTEX",attr_name:str=None,schema_name:str=None):
        ...
        if attr_name:
            _ = self._add_attribute(schema_type,self.queryResult_type_dict[query_name],attr_name,schema_name)
        return resp.strip() 
    ...
    def _get_Params(self,name_of_query:str):
        '''
        return default query parameter by parsing the query header
        '''
        self.local_gsql_path = pjoin(os.path.dirname(
            os.path.abspath(__file__)), "gsql", "featurizer", name_of_query + ".gsql")
        with open(self.local_gsql_path) as infile:
            _dict = {}
            query = infile.read()
        try:
    ...
    def runAlgorithm(self,name_of_query:str,params:dict = None, timeout:int=2147480) -> None:
        ...
        if params == None:
            ...
            if 'v_type' in params.values():
                params['v_type'] = self.conn.getVertexTypes()[0]
            if 'e_type' in params.values():
                params['e_type'] = self.conn.getEdgeTypes()[0]
